...
~CapsLock::     ;<==达到一键开关效果，但是丢失大小写功能。还可用NumLock或ScrollLock的状态，替换脚本中【24/25/242/254行】的CapsLock即可。by Zz
...
`::     ;<==自定义修改此键开启hint模式，因为连续hint模式要实时检测状态，关闭仍然要按CapsLK而非自定义键，但注释掉上一种方法就不丢失CapsLK原来大小写功能
...
az:="q|a|z|w|s|x|e|d|c|r|f|v|t|g|b|y|h|n|u|j|m|i|k|o|l|p"   ;<==【重排26字母为键盘竖排从左到右】
azList:=StrSplit(az,"|")
;===== Display qwer
if(linesSize>26)
{
  qwer:="q|a|z|w|s|x|e|d|c|r|f|v"   ;<==【顺序数量按自己喜好添加】
  qwerStr:=StrSplit(qwer,"|")
  qwerList:=Object()
  Loop,% qwerStr.MaxIndex()
  {
      Z_Index:=A_Index
      Loop,% qwerStr.MaxIndex()
      {
          qwerList.Insert(qwerStr[Z_Index] qwerStr[A_Index])
      }
  }
}
;~ MsgBox,% qwerList.MaxIndex()
;----- Display numbers
counter=-1
Loop,Parse,lines,`n
{
  ...
  ;~ if(counter<9){
    ;~ counter+=1
  if(linesSize<=26 && A_Index<linesSize){
    counter:=azList[A_Index]
  }else if(linesSize>26 && A_Index<linesSize){
    counter:=qwerList[A_Index]
  }
...
Loop
{
  ...
  Loop,%linesSize%
  {
    ...
    ;~ if(digit<9){
      ;~ digit+=1
    if(linesSize<=26 && A_Index<azList.MaxIndex()){
      digit:=azList[A_Index]
    }else if(linesSize>26 && A_Index<qwerStr.MaxIndex()){
      digit:=qwerStr[A_Index]
    }
  }
